...
<meta name="twitter:site" content="@LingoIO" />
<meta name="twitter:creator" content="@LingoIO" />
<meta name="twitter:owner" content="@LingoIO" />
<meta property="twitter:description" content="Learn and practice spanish conversation with speech technologies || #OpenSource" />
<meta property="twitter:title" content="ÜberSnip - AdVoice WebNavigation. || @LingoIO App" />
...
<meta property="og:description" content="Learn and practice spanish conversation with speech technologies || #OpenSource" />
	...
	<body class="no-trans scroll-spy"><div class="modal fade" id="project-1" tabindex="-1" role="dialog" aria-labelledby="project-1-label" aria-hidden="true">
								...
								</div><div class="modal fade" id="project-6" tabindex="-1" role="dialog" aria-labelledby="project-6-label" aria-hidden="true">
									<div class="modal-dialog modal-lg">
										<div class="modal-content">
											<div class="modal-header">
												<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span><span class="sr-only">Close</span></button>
								...
								</div><div class="modal fade" id="project-8" tabindex="-1" role="dialog" aria-labelledby="project-8-label" aria-hidden="true">
									<div class="modal-dialog modal-lg">
										<div class="modal-content">
											<div class="modal-header">
												<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span><span class="sr-only">Close</span></button>
								...
								</div><div class="modal fade" id="project-9" tabindex="-1" role="dialog" aria-labelledby="project-9-label" aria-hidden="true">
									<div class="modal-dialog modal-lg">
										<div class="modal-content">
											<div class="modal-header">
												<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span><span class="sr-only">Close</span></button>
								...
								</div><div class="modal fade" id="project-10" tabindex="-1" role="dialog" aria-labelledby="project-10-label" aria-hidden="true">
									<div class="modal-dialog modal-lg">
										<div class="modal-content">
											<div class="modal-header">
												<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span><span class="sr-only">Close</span></button>
								...
								</div><div class="modal fade" id="project-12" tabindex="-1" role="dialog" aria-labelledby="project-12-label" aria-hidden="true">
									<div class="modal-dialog modal-lg">
										<div class="modal-content">
											<div class="modal-header">
												<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span><span class="sr-only">Close</span></button>
		...
		recognizer.onresult = function(event) {
			if (event.results.length > 0) {
				...
				if(result.isFinal) {
					...
					answer_question (result[0].transcript);
					
					if ( result[0].transcript.indexOf( 'siguiente' ) >= 0 ){
						next_question();	
					}
					
					if ( result[0].transcript.indexOf( 'reiniciar' ) >= 0 ){
						restart_lesson();	
					}
					
					if ( result[0].transcript.indexOf( 'comenzar' ) >= 0 && working == false){
						restart_lesson();	
					}
				}
			}  
		...
		$.get("https://ubersnip.com/UberSnip_VoiceAPI/LingoIO/api/lessonData.php?lesson_id=1", function(data){
			questions = data;
		});
		...
		var questions = JSON.parse('[{"ID":"1","lesson_id":"1","lesson_speech":"Hola, \u00bfc\u00f3mo est\u00e1s?","response_fwd":"2","hint":"<big><strong>How would you respond to this?<\/strong><\/big>"},{"ID":"2","lesson_id":"1","lesson_speech":"Bueno. Mi nombre es Uber. \u00bfC\u00f3mo te llamas?","response_fwd":"3","hint":"<big><strong>How would you respond to this?<\/strong><\/big>"}]');
		var question_index = 0;
		var working = false;
		var inAnswerMode = false;
		var game_done = false;
		
		function next_question(response){
			if ( working || game_done ){ return; }
			
			inAnswerMode = true;
			$(".game_header").html(questions[question_index].lesson_speech);
			$(".game_section_text").html(questions[question_index].hint);
			$(".start_button").html("Next");
		}
		
		function restart_lesson(){
			question_index = 0;
			game_done=false;
			next_question();	
		}
			...
			$.get("api/request.php", {sentence_id:questions[question_index].ID, req: answer }).done(function(data){
				...
				if(data.correct){
					...
					if ( question_index >= questions.length ){
						...
						inAnswerMode = false;
						$(".game_section_text").html("<font color='green'><strong>Great Job!</strong> All finished with lesson!</font>");
					}
				}else{
					inAnswerMode = true;
				}
	...
	<div class="keyword_detection"></div>
